...
#' @param alluvial.x.axis.text.position Location of x-axis labels in alluvial plot
...
tcm_alluvial_dot <- function(data,
  ...
  df <- data_alluvial %>% ggsankey::make_long(colnames(data_alluvial))

  if (dot.lable == "Description" && molecule.lable == "molecule_id") {
    df$node <- factor(df$node, levels = c(
      data_alluvial$Description %>% unique(),
      data_alluvial$target %>% unique(),
      data_alluvial$molecule_id %>% unique(),
      data_alluvial$herb %>% unique()
    ))
  } else if (dot.lable == "ID" && molecule.lable == "molecule_id") {
    df$node <- factor(df$node, levels = c(
      data_alluvial$ID %>% unique(),
      data_alluvial$target %>% unique(),
      data_alluvial$molecule_id %>% unique(),
      data_alluvial$herb %>% unique()
    ))
